...
def test():
    ...
    for x in range(100000):
        if (random.random() < 0.5):
            random_input = get_random_int()
            
            if len(shaddow_queue) >= 50000:
                assert q.enqueue(random_input) is False
                assert q.full() is True
            else:
                assert q.enqueue(random_input) is True
                shaddow_queue.append(random_input)
        else:
            if len(shaddow_queue) > 0:
                shadow_result = shaddow_queue.pop(0)
                real_result = q.dequeue()
                assert real_result == shadow_result
            else:
